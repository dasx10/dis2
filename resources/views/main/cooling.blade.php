...
@section('script')
    ...
    <script>
        ...
        $('.titles{{$key}}').webuiPopover({placement:'bottom',trigger: "hover",content:'<div class="text-center"><img class="img-fluid" src="{{$slide->src1}}" alt=""> <p>'+descr+'</p><button data-link="{{$slide->link}}" onclick="redirect(this)" class="btn btn-primary btn-view">Visit website</button></div>'});
        @endforeach

        function redirect(btn) {
            window.location = $(btn).attr('data-link');
        }
