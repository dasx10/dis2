...
//                DefaultValidation::check_token($data['token']);
                ...
                $request->full_pass =  $insert_data['password'];
        ...
        private function get_created_by_id($token){
            return DB::table('sessions')->where('token','=',$token)->value('admins_id');
        }
        ...
        private function check_empty_data($data){
            $insert_data = [];
            $array_required = ['password'=>'password','contact_name'=>'contact name',
                'email'=>'email','company_name'=>'company name','phone_number'=>'phone number',
                'regione'=>'regione','country'=>'country'];
